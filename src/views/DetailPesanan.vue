<template>
  <v-container class="my4 md5">
    <v-card class="justify-center">
      
      <v-card>
        <h1 class="text-center">Detail Pesanan</h1>
        <v-card-title>
          <v-text-field
            v-model="search"
            append-icon="mdi-magnify"
            label="Search"
            single-line
            hide-details
          ></v-text-field>
          <v-spacer></v-spacer>
        </v-card-title>
        <v-data-table :headers="headers" :items="detailpesanans" :search="search">
          <template v-slot:[`item.actions`]="{ item }">
            <v-btn
              small
              class="mr-2"
              @click="deleteHandler(item)"
            >
              Selesai
            </v-btn>
          </template>
        </v-data-table>
      </v-card>
      <v-snackbar v-model="snackbar" :color="color" timeout="2000" bottom>{{
        error_message
      }}</v-snackbar>
    </v-card>
  </v-container>
</template>
<script>
export default {
  name: "detailpesanan",
  data() {
    return {
      inputType: "Tambah",
      load: false,
      snackbar: false,
      error_message: "",
      color: "",
      search: null,
      dialog: false,
      dialogConfirm: false,

      headers: [
        {
          text: "No Pesanan",
          align: "start",
          sortable: true,
          value: "id_pesanan",
        },
        { text: "Nama Menu", value: "nama_menu" },
        { text: "Jumlah Pesanan", value: "jumlah_pesanan" },
        { text: '', value: 'actions'},
      ],

      detailpesanan: new FormData(),
      detailpesanans: [],
      menus: [],
      deleteId: "",
      editId: "",
    };
  },

  watch:{
    dialog(val) {
      val || this.close()
    },
  },
  methods: {
    setForm() {
      if (this.inputType === "Tambah") {
        this.save();
      } else {
        this.update();
      }
    },
    //read data
    readData() {
      var url = this.$api + "/detailpesanan/tampil/";
      this.$http
        .get(url, {
          headers: {
            Authorization: "Bearer" + localStorage.getItem("token"),
          },
        })
        .then((response) => {
          this.detailpesanans = response.data.data;
        });
    },
    readdetailPesanan() {
      var url = this.$api + "/detailpesanan/tampil/";
      this.$http
        .get(url, {
          headers: {
            Authorization: "Bearer" + localStorage.getItem("token"),
          },
        })
        .then((response) => {
          this.detailpesanans = response.data.data;
          console.log(this.detailpesanans);
        });
    },

    save() {
      this.karyawan.append("nama_karyawan", this.form.nama_karyawan);
      this.karyawan.append("no_telepon", this.form.no_telepon);
      this.karyawan.append("email_karyawan", this.form.email_karyawan);
      this.karyawan.append("password", this.form.password);
      var url = this.$api + "/karyawan/tambah/";
      this.load = true;
      this.$http
        .post(url, this.karyawan, {
          headers: {
            Authorization: "Bearer" + localStorage.getItem("token"),
          },
        })
        .then((response) => {
          console.log(response);
          this.error_message = response.data.message;
          this.color = "green";
          this.snackbar = true;
          (this.load = false), this.close();
          this.readData();
          this.resetForm();
        })
        .catch((error) => {
          console.log(error);
          this.error_message = error.response.data.message;
          this.color = "red";
          this.snackbar = true;
          this.load = false;
        });
    },

    //update
    update() {
      let newData = {
        nama_karyawan: this.form.nama_karyawan,
        no_telepon: this.form.no_telepon,
        email_karyawan: this.form.email_karyawan,
      };
      var url = this.$api + "/karyawan/update/" + this.editId;
      this.load = true;
      this.$http
        .put(url, newData, {
          headers: {
            Authorization: "Bearer" + localStorage.getItem("token"),
          },
        })
        .then((response) => {
          this.error_message = response.data.message;
          this.color = "green";
          this.snackbar = true;
          this.load = false;
          this.close();
          this.readData(); //mengambil data
          this.resetForm();
          this.inputType = "Tambah";
        })
        .catch((error) => {
          this.error_message = error.response.data.message;
          this.color = "red";
          this.snackbar = true;
          this.load = false;
        });
    },

    //hapus
    deleteData() {
      //menghapus data
      var url = this.$api + "/detailpesanan/hapus/" + this.deleteId;
      //data dihapus berdasarkan id
      this.$http
        .get(url, {
          headers: {
            Authorization: "Bearer" + localStorage.getItem("token"),
          },
        })
        .then((response) => {
          this.error_message = response.data.message;
          this.color = "green";
          this.snackbar = true;
          this.load = false;
          this.readdetailPesanan(); //mengambil data
          this.inputType = "Tambah";
        })
        .catch((error) => {
          this.error_message = error.message;
          this.color = "red";
          this.snackbar = true;
          this.load = false;
        });
    },
    aktifData() {
      //menghapus data
      var url = this.$api + "/karyawan/aktif/" + this.deleteId;
      //data dihapus berdasarkan id
      this.$http
        .get(url, {
          headers: {
            Authorization: "Bearer" + localStorage.getItem("token"),
          },
        })
        .then((response) => {
          this.error_message = response.data.message;
          this.color = "green";
          this.snackbar = true;
          this.load = false;
          this.close();
          this.readData(); //mengambil data
          this.resetFrom();
          this.inputType = "Tambah";
        })
        .catch((error) => {
          this.error_message = error.response.data.message;
          this.color = "red";
          this.snackbar = true;
          this.load = false;
        });
    },
    editHandler(item) {
      this.inputType = "Ubah";
      this.editId = item.id;
      this.form.nama_karyawan = item.nama_karyawan;
      this.form.no_telepon = item.no_telepon;
      this.form.email_karyawan = item.email_karyawan;
      this.dialog = true;
      this.status = true;
    },
    deleteHandler({ id }) {
      this.deleteId = id;
      this.deleteData();
    },

    close() {
      this.dialog = false;
      this.inputType = "Tambah";
      this.status = false;
      this.form = {};
    },

    cancel() {
      this.dialog = false;
      this.resetForm = {
        nama_karyawan: null,
        no_telepon: null,
        email_karyawan: null,
      };
    },
  },
  computed: {
    formTitle() {
      return this.inputType;
    },
  },
  mounted() {
    this.readdetailPesanan();
    this.readData();
  },
};
</script>